...
class PoliceStation:
    ...
    def __init__(self, station_data: Dict[str, Any]):
        ...
        self.response_time_minutes = station_data.get('response_time_minutes', 10)
        self.operational_hours = station_data.get('operational_hours', '24/7')
        self.specialties = station_data.get('specialties', [])
        self.jurisdiction = station_data.get('jurisdiction', '')
        self.distance_km = None

class PoliceEmergencyCoordinator:
    def __init__(self):
        """Initialize Police Emergency Coordinator"""
    ...
    def _load_police_stations(self) -> List[PoliceStation]:
        """Load police stations from JSON file"""
        stations = []
        try:
            with open('data/karachi_police_services.json', 'r', encoding='utf-8') as f:
                data = json.load(f)
                for station_data in data:
                    station = PoliceStation(station_data)
                    stations.append(station)
                print(f"Loaded {len(stations)} police stations and units")
                return stations
        except FileNotFoundError:
            print("Warning: Police services data not found")
            return []
        except Exception as e:
            print(f"Error loading police stations: {e}")
            return []
    
    async def process_police_emergency_request(self, message: str, user_ip: str = None, 
                                             session_id: Optional[str] = None) -> ChatResponse:
        """
        Process police/emergency service request
        """
        if not session_id:
        ...
        try:
            ai_response = await self.gemini_client.chat_with_police_emergency(
                message=message,
                context=context,
                stations=stations,
    ...
    def _detect_emergency_type(self, message: str) -> str:
        ...
        emergency_keywords = {
            'crime': ['robbery', 'theft', 'burglary', 'mugging', 'snatching', 'crime', 'criminal', 'stolen'],
            'violence': ['fight', 'violence', 'assault', 'attack', 'threat', 'harassment', 'abuse'],
            'traffic': ['accident', 'traffic', 'road', 'vehicle', 'collision', 'crash', 'hit and run'],
            'domestic': ['domestic', 'family', 'spouse', 'partner', 'children', 'abuse', 'violence'],
            'cyber': ['cyber', 'online', 'fraud', 'scam', 'hacking', 'identity', 'internet'],
            'drug': ['drug', 'narcotics', 'addiction', 'dealer', 'substance', 'illegal'],
            'terrorism': ['terrorist', 'bomb', 'explosive', 'threat', 'suspicious', 'security'],
            'kidnapping': ['kidnapping', 'abduction', 'missing', 'kidnapped', 'taken'],
            'women_safety': ['women', 'harassment', 'stalking', 'eve teasing', 'molestation'],
            'child_protection': ['child', 'minor', 'abuse', 'exploitation', 'protection']
        }
    ...
    def _get_relevant_stations(self, session, emergency_type: str = None) -> List[Dict]:
        """Get relevant police stations based on location and emergency type"""
        stations = []
        ...
        if session and session.location and session.location.latitude and session.location.longitude:
            ...
            # If no location, get top police stations by type
            if emergency_type:
                relevant_stations = self._find_emergency_stations(emergency_type)
            else:
                relevant_stations = self.police_stations[:5]
    ...
    def _find_nearby_stations(self, lat: float, lon: float, radius_km: float = 15.0) -> List[PoliceStation]:
        """Find police stations within specified radius"""
        nearby_stations = []
    ...
    def _find_emergency_stations(self, emergency_type: str = None, lat: float = None, lon: float = None) -> List[PoliceStation]:
        """Find stations suitable for emergency type"""
        relevant_stations = []
        
        # Map emergency types to station types and specialties
        type_mapping = {
            'crime': ['police_station', 'special_investigation', 'anti_car_lifting'],
            'violence': ['police_station', 'rapid_response', 'women_police'],
            'traffic': ['traffic_police'],
            'domestic': ['women_police', 'police_station'],
            'cyber': ['special_investigation'],
            'drug': ['narcotics_unit'],
            'terrorism': ['rapid_response', 'special_investigation'],
            'kidnapping': ['special_investigation', 'rapid_response'],
            'women_safety': ['women_police', 'police_station'],
            'child_protection': ['women_police', 'special_investigation']
        }
        
        target_types = type_mapping.get(emergency_type, ['police_station'])
        
        for station in self.police_stations:
            # Check station type
            if station.type in target_types:
                relevant_stations.append(station)
            # Also check specialties for better matching
            elif emergency_type and self._is_relevant_specialty(station.specialties, emergency_type):
                relevant_stations.append(station)
    ...
    def _is_relevant_specialty(self, station_specialties: List[str], emergency_type: str) -> bool:
        """Check if station specialty is relevant to emergency type"""
        emergency_type = emergency_type.lower()
        specialties = [spec.lower() for spec in station_specialties]
        
        # Map emergency types to relevant specialties
        specialty_mapping = {
            'crime': ['crime investigation', 'criminal investigation', 'investigation'],
            'violence': ['rapid response', 'public order', 'crisis management'],
            'traffic': ['traffic management', 'accident investigation', 'traffic control'],
            'domestic': ['women safety', 'domestic violence', 'family protection'],
            'cyber': ['cyber crime', 'investigation', 'special investigation'],
            'drug': ['drug enforcement', 'narcotics investigation'],
            'terrorism': ['special operations', 'crisis management', 'rapid response'],
            'kidnapping': ['investigation', 'special operations', 'rapid response'],
            'women_safety': ['women safety', 'women crime investigation', 'domestic violence'],
            'child_protection': ['child protection', 'women crime investigation']
        }
        
        if emergency_type in specialty_mapping:
            for keyword in specialty_mapping[emergency_type]:
                if any(keyword in spec for spec in specialties):
                    return True
    ...
    def _needs_emergency_confirmation(self, message: str, ai_response: str) -> bool:
        ...
        emergency_keywords = ['police', 'emergency', 'urgent', 'help', 'crime', 'violence', 'threat', 'danger']
        message_lower = message.lower()
        ...
        ai_suggests_emergency = any(keyword in ai_response.lower() for keyword in ['emergency', 'dispatch', 'station', 'response', 'police'])
